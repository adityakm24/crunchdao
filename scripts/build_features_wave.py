"""Build the features_wave labeled matrix -> features/train_features_wave.npz.

Same protocol as build_features4.py. Run sharded to bound RAM:
  for k in 0 1; do uv run python scripts/build_features_wave.py --shard $k --n-shards 4 & done; wait
  for k in 2 3; do uv run python scripts/build_features_wave.py --shard $k --n-shards 4 & done; wait
  uv run python scripts/build_features_wave.py --merge --n-shards 4
"""
...
from sb.features_wave import StreamingDetector, FEATURE_NAMES, N_FEATURES

OUT = "features"
NAME = "train_features_wave"
