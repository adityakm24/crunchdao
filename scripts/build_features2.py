"""Build the v2 labeled feature matrix (features2.StreamingDetector).

Identical protocol to build_features.py but with the v2 extractor and output
features/train_features_v2.npz (NOT committed; .npz are LFS-tracked and the
remote proxy rejects LFS pushes -- regenerate with this script instead).

Run:  uv run python scripts/build_features2.py                  # single process
      uv run python scripts/build_features2.py --shard 0 --n-shards 4   # parallel
      uv run python scripts/build_features2.py --merge --n-shards 4     # combine
"""
...
from sb.features2 import StreamingDetector, FEATURE_NAMES, N_FEATURES
...
def merge(n_shards: int) -> None:
    ...
    for k in range(n_shards):
        d = np.load(os.path.join(OUT, f"train_features_v2.shard{k}.npz"),
                    allow_pickle=True)
    ...
    np.savez(os.path.join(OUT, "train_features_v2.npz"),
    ...
    for k in range(n_shards):
        os.remove(os.path.join(OUT, f"train_features_v2.shard{k}.npz"))
...
def main() -> None:
    ...
    np.savez(
        os.path.join(OUT, f"train_features_v2{suffix}.npz"),
        X=X, y=y, series_id=sid, t_online=t_online,
        feature_names=np.array(FEATURE_NAMES),
    )
    print(f"Saved {OUT}/train_features_v2{suffix}.npz in {time.time()-t0:.1f}s total")
