...
## 2. Current model (round 4 — best, ready to submit)

- **Held-out VAL TS-AUC = 0.6161** (2000-series internal split), up from round-3's
  0.6041 (**+1.2 pts**) and the EWMA baseline 0.4806 (**+13.5 pts**). This
  **clears the leaderboard top-10 cutoff (~0.6135)** on the internal split.
- **Model:** `submission/main.py` (generated by `scripts/make_submission4.py`) —
  ...
  - **GRU neural sub-ensemble (the round-4 lift):** 3 single-layer GRUs
    (hidden 128/96/160, seeds 0/1/2) over the 152-feature calibrated stream, their
    step-logits **averaged** into one neural member and mixed in as
    `final = 0.6·base + 0.4·mean(GRU logits)`. The recurrent net integrates break
    evidence over time — a different inductive bias from trees (rank-corr ~0.83 vs
