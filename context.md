...
## 2. Current model (round 3 — best, ready to submit)

- **Held-out VAL TS-AUC = 0.6041** (2000-series internal split), up from round-2's
  0.5812 (**+2.3 pts**) and the EWMA baseline 0.4806 (**+12.4 pts**).
- **Model:** `submission/main.py` (generated by `scripts/make_submission3.py`) —
  ...
  - **Ensemble** (equal **mean-logit**): 4 LightGBM members — 3 on the v2 feature
    ...
    corr 0.93). `num_leaves=31, lr=0.03, min_data=1000, deterministic=True`.
...
- **Local verification:** smoke test **determinism max|diff| = 0.0** (≪1e-8),
  ~0.4 ms/point → **~48 min** for the full 10k-series test (≪ 15 h budget).
  Full `submission/local_test.py` mirrors the cloud runner end-to-end.
- **Prior live submission:** #1 (round-2 model, VAL 0.5812) to project
  `chinchilla` (userId 13086). The round-3 model above supersedes it — push it
  next (see §3 "Submitting again").
...
- Round-3 map: `model_002` reproduces round 2; `model_003`=v2; `010-012`=v3;
  ...
  `019`=v5 (LRV calib, rejected); **`model_018_ensemble`=the shipped FINAL**
  (recipe + member models + main.py).
...
calibration. We are now near a **new ceiling for GBT ensembling**: all 10 trained
GBTs correlate **0.97–0.99** (`scripts/ensemble_search.py`), so averaging them
caps ~0.602; the logistic member is what pushes to 0.604. Structural reasons the
metric stays hard:
...
- `tau ~ Uniform` over the online segment → at any step a large share of "broken"
  ...
  genuine ceiling (`scripts/diagnose.py`).
- **Subtle/distribution-only breaks are 68% of breaks and only ≈0.56 AUC**
  (median KS ≈ 0.12, near the two-sample detection limit). The dominant
  *addressable* weakness; v4's calibrated skew/kurt nudged but didn't crack it.

**Leaderboard context:** top-10 cutoff ≈ **0.6135**, #1 ≈ 0.6322. We are **~+0.9
pt short**. Closing it needs a genuinely new representation, not more GBT tuning.
...
> the AR(1)-prewhitened bank already does it). So the next pts are unlikely to
> come from more handcrafted streaming features; they need a different *model*.

1. **A neural sequence member** (tiny temporal CNN/GRU over the calibrated
   feature stream, or a 1-D CNN on the raw online window). The GBT ensemble is
   saturated (corr 0.97–0.99); the logistic (corr 0.93, +0.001) showed that
   *model-class* decorrelation is what's left. A neural member is the natural,
   stronger version of that. Seed + single-thread for determinism. **Highest EV.**
2. **TSFM drift embeddings** (Chronos/Moirai/TimesFM), historical-vs-trailing
   ...
   for the 15 h budget. Biggest potential lift, biggest effort — do last.
3. **Per-break-type experts + gate** (mean/var/acf/subtle), à la the 2025 winners'
   epistemic diversity. More plumbing; the subtle-break expert is the one that
   matters (68% of breaks, 0.56 AUC).
...
Learned stacking; leaves>31 / aggressive feature_fraction / per-step weighting /
scale_pos_weight; score postprocessing (running-max/smoothing); raw uncalibrated
distance/higher-moment features; **LRV-calibrated cumulative detectors (v5)**.
All measured to hurt or no-op.
