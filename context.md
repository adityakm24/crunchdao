...
- **Round 7 (TSFM embedding — the 7th negative, ships nothing).** Tested the
  "different data source" hypothesis directly: a real **time-series foundation
  model** (Amazon **Chronos-T5-mini**, 20M-param T5 encoder, 384-dim embedding)
  embedding the historical (break-free) vs trailing/online windows, exactly the
  blueprint the competition info doc proposes. Probed at the metric-heavy snapshot
  steps with three escalating forms, each gated for *incremental* lift onto the
  shipped 0.6160 stack: (1) **embedding distance** (cosine/L2 hist-vs-cum/trail,
  EOS-token pooling) — all features **~0.50 standalone**, max blend lift **+0.0018**;
  (2) **supervised logistic head** on the 1152-dim embedding-difference vectors
  (grouped 5-fold CV) — head AUC **~0.515**, blend lift **+0.0008–0.0011**, but
  **rank-corr 0.40–0.52** to the shipped logit (the tiny signal is *already
  captured*); (3) **mean-pooled** embeddings (length-bucketed, exact) re-running
  both — even flatter (**distance lift +0.0000–0.0003**, head AUC **0.49–0.50**,
  lift **+0.0000**). **Verdict: the Chronos embedding signal is both tiny AND
  redundant**, across {distance, supervised} × {EOS, mean} pooling. A bigger
  Chronos (small/base) won't help — the mini's signal is not weak-but-orthogonal,
  it is weak-and-already-captured — and the offline-numpy-T5 deployment cost is
  enormous for sub-noise gain. Scripts: `eda_tsfm.py`, `eda_tsfm_head.py`,
  `eda_tsfm_mean.py` (Chronos installed **local-only**; weights NOT bundled).
  **Verdict after 7 independent signal classes: the model is comprehensively
  saturated on the available per-series data.** The ~0.6322 gap is not reachable
  by adding a member to this representation; it would need a different label
  structure or a cross-series/regime model — beyond the current architecture.
...
2. **TSFM drift embeddings** (Chronos/Moirai/TimesFM) — **TESTED & REJECTED in
   round 7.** Chronos-T5-mini embedding distance ~0.50 standalone; a supervised
   head reaches ~0.515 but rank-corr 0.40–0.52 (redundant); mean-pooling even
   flatter. The embedding adds no orthogonal cross-sectional signal. Do not
   re-probe with a bigger Chronos — the signal is already captured, not just weak.
3. **Per-break-type experts + gate** (mean/var/acf/subtle), à la the 2025 winners'
   ...
   breaks, 0.56 AUC) — the GRU integrates evidence but doesn't specialise. NOTE:
   round 6 already showed the residual subtle cohort sits at the ≤0.50 detection
   floor, so a dedicated expert is unlikely to clear the bar either.
4. **Energy distance / MMD + spectral-slope**, calibrated per series. Lower EV:
   KS/L1/skew/kurt are already calibrated and correlated with these. (Round 6
   tested AD/energy/CvM/Wasserstein/Hurst — +0.0005, weight 0.)
...
**Round-6 additions (6 negatives):** subtle-break detectors (cohort ≤0.50 floor);
distributional distances AD/energy/CvM/Wasserstein/Hurst (+0.0005); Shiryaev–Roberts
(+0.001); ExtraTrees/RandomForest (rank-corr 0.958 — redundant); raw matched filters
(+0.0017); 1-D CNN over raw stream (most decorrelated ever, rank-corr 0.77, but
flat→declining). **Round-7 addition (7th negative):** **TSFM (Chronos-T5-mini)
embeddings** — distance ~0.50, supervised head ~0.515 (rank-corr 0.40–0.52,
redundant), mean-pool even flatter. Do NOT re-probe a bigger Chronos / other TSFM:
the embedding signal is *already captured* by the stack, not weak-but-orthogonal.
**After 7 signal classes the model is comprehensively saturated on the per-series
inputs.**
